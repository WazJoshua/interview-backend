...
@Repository
public interface BillingPlanVersionRepository extends JpaRepository<BillingPlanVersion, Long> {

    @Query(
            value = """
                    SELECT *
                    FROM billing_plan_version version
                    WHERE version.billing_plan_id = :billingPlanId
    ...
    List<BillingPlanVersion> findOverlappingVersions(
    ...
    Optional<BillingPlanVersion> findActiveSellableVersion(
            @Param("billingPlanId") Long billingPlanId,
